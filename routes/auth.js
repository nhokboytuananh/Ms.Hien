...
const JWT_SECRET =
  process.env.JWT_SECRET || "giao_vien_tieng_anh_ms_hien_bi_mat_123";
...
router.post("/register", async (req, res) => {
  ...
  if (role === "teacher") {
    return res
      ...
      .json({
        error:
          "Hệ thống chỉ cho phép duy nhất một tài khoản Giáo viên mặc định.",
      });
  }
  ...
  try {
    ...
    if (checkUser.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "Email này đã được sử dụng bởi một tài khoản khác." });
    }
    ...
    res
      .status(500)
      .json({ error: "Đã xảy ra lỗi trên hệ thống. Vui lòng thử lại sau." });
  }
});
...
router.post("/login", async (req, res) => {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .json({ error: "Vui lòng điền đầy đủ email và mật khẩu." });
  }
  ...
  try {
    ...
    if (user.is_locked) {
      return res
        ...
        .json({
          error:
            "Tài khoản của bạn đã bị khóa. Vui lòng liên hệ giáo viên để được mở khóa.",
        });
    }
    ...
    if (!isMatch) {
      return res
        .status(400)
        .json({ error: "Mật khẩu không chính xác. Vui lòng thử lại." });
    }
    ...
    res
      .status(500)
      .json({ error: `Đã xảy ra lỗi trên hệ thống: ${error.message}` });
  }
});
...
router.post("/forgot-password", async (req, res) => {
  ...
  try {
    ...
    if (userRes.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "Không tìm thấy tài khoản nào khớp với email này." });
    }
    ...
    res
      .status(500)
      .json({ error: "Không thể xử lý yêu cầu khôi phục mật khẩu." });
  }
});
