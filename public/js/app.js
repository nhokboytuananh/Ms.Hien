...
window.apiFetch = async (endpoint, options = {}) => {
  const headers = {
    ...
    ...(window.appState.token ? { 'Authorization': `Bearer ${window.appState.token}` } : {}),
    ...(options.headers || {})
  };
...
window.showConfirm = (options = {}) => {
  return new Promise((resolve) => {
    ...
    if (options.type === 'danger') {
      ...
      okBtn.className = 'px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl text-xs transition-all cursor-pointer';
    } else {
      ...
      okBtn.className = 'px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-xs transition-all cursor-pointer';
    }
...
function showAuthAlert(message, type = 'error') {
  ...
  alertEl.className = `p-4 mb-4 rounded-lg text-sm font-semibold text-center ${
    type === 'error' ? 'bg-rose-100 text-rose-800 border border-rose-200' : 'bg-emerald-100 text-emerald-800 border border-emerald-200'
  }`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadPublicClasses() {
    ...
    try {
      ...
      regClassSelect.innerHTML = '<option value="">-- Lỗi tải lớp, vui lòng thử lại --</option>';
    }
  }
  ...
  tabLogin.addEventListener('click', () => {
    tabLogin.className = "flex-1 pb-3 text-center font-semibold text-brand-600 border-b-2 border-brand-600 focus:outline-none";
    tabRegister.className = "flex-1 pb-3 text-center font-medium text-slate-400 border-b-2 border-transparent hover:text-slate-600 focus:outline-none";
    loginForm.classList.remove('hidden');
    registerForm.classList.add('hidden');
  });
 
  tabRegister.addEventListener('click', () => {
    tabRegister.className = "flex-1 pb-3 text-center font-semibold text-brand-600 border-b-2 border-brand-600 focus:outline-none";
    tabLogin.className = "flex-1 pb-3 text-center font-medium text-slate-400 border-b-2 border-transparent hover:text-slate-600 focus:outline-none";
  ...
  registerForm.addEventListener('submit', async (e) => {
    ...
    const class_name = role === 'student' ? document.getElementById('reg-class').value.trim() : null;
 
    if (role === 'student' && !class_name) {
      showAuthAlert('Vui lòng chọn một lớp học do Giáo viên đã lập sẵn!', 'error');
      return;
    }
...
async function initDashboard() {
  ...
  if (user.role === 'teacher') {
    ...
    roleBadge.className = 'ml-2 px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-brand-100 text-brand-700 uppercase tracking-wider';
    ...
    roleBadge.className = 'ml-2 px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-emerald-100 text-emerald-700 uppercase tracking-wider';
...
function setupTabListeners(navContainerId, defaultTabId) {
  ...
  buttons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      buttons.forEach(b => {
        b.classList.remove('bg-brand-50', 'text-brand-700', 'bg-brand-500/10', 'text-brand-400', 'font-bold');
  ...
  const defaultBtn = container.querySelector(`[data-tab="${defaultTabId.replace('view-', '')}"]`);
  if (defaultBtn) {
    defaultBtn.click();
  }
}
...
function triggerTabLoadData(tabName) {
  // GIÁO VIÊN
  if (tabName === 't-classes' && typeof window.loadTeacherClasses === 'function') {
    window.loadTeacherClasses();
  }
  if (tabName === 't-vocab' && typeof window.loadTeacherVocabulary === 'function') {
...
window.showExamLeaderboard = async (examId, examTitle) => {
  ...
  modal.classList.remove('hidden');
  modal.classList.add('flex');
  document.getElementById('leaderboard-modal-title').textContent = `Bảng Xếp Hạng Kỷ Lục (Top 5): ${examTitle || 'Thi Thử'}`;
