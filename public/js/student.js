...
window.switchVocabMode = (mode) => {
  ...
  if (mode === "flashcard") {
    btnFc.className =
      "px-4 py-1.5 rounded-lg text-xs font-bold transition-all bg-white text-slate-800 shadow";
    btnGame.className =
...
window.startFlashcardStudy = () => {
  ...
  const filtered = window.studentState.vocabList.filter(
...
function renderFlashcard() {
  ...
  document
    .getElementById("flashcard-container")
    .classList.remove("rotate-y-180");

  document.getElementById("fc-badge-topic").textContent =
    `Chủ đề: ${word.topic || "Chung"}`;
...
window.startGame = (gameType) => {
  ...
  if (grade === "all") {
    ...
    words = (window.studentState.vocabList || []).filter(
      (v) => v.grade === Number(grade),
    );
  }
  ...
  if (words.length < 4 && gameType === "quiz") {
    alert(
      "Khối lớp hiện tại chưa có đủ từ vựng (tối thiểu 4 từ) để tạo đáp án trắc nghiệm. Con vui lòng chọn lớp khác hoặc nhắc Cô Hiền thêm từ vựng nhé!",
    );
    return;
  }
  if (words.length === 0) {
    alert(
      "Khối lớp hiện tại chưa có từ vựng để chơi game. Con vui lòng chọn lớp khác nhé!",
    );
    return;
  }
  ...
  window.studentState.gameStartTime = Date.now();
...
function loadQuizQuestion(index) {
  ...
  options.forEach((opt, idx) => {
    ...
    btn.className =
      "quiz-opt-btn w-full p-4 text-slate-800 bg-white border border-slate-200 hover:border-brand-500 hover:bg-brand-50/20 rounded-2xl font-bold text-sm transition-all shadow-sm text-left flex items-center justify-between cursor-pointer";
    btn.innerHTML = `
      <span>${opt}</span>
  ...
  document.getElementById("quiz-feedback").className =
    "hidden p-4 rounded-xl text-sm font-bold text-center";
...
window.submitQuizAnswer = (chosenOption, clickedBtn) => {
  ...
  buttons.forEach((btn) => {
    ...
    if (text.toLowerCase() === correctWord.toLowerCase()) {
      ...
      btn.className =
        "quiz-opt-btn w-full p-4 text-emerald-800 bg-emerald-50 border-2 border-emerald-500 rounded-2xl font-bold text-sm flex items-center justify-between";
      ...
      btn.className =
        "quiz-opt-btn w-full p-4 text-rose-800 bg-rose-50 border-2 border-rose-500 rounded-2xl font-bold text-sm flex items-center justify-between";
  ...
  if (isCorrect) {
    ...
    document.getElementById("quiz-correct-count").textContent =
      window.studentState.gameCorrectAnswers;
...
function loadScrambleQuestion(index) {
  ...
  document.getElementById("scramble-progress-text").textContent =
    `Câu ${index + 1}/${total}`;
  ...
  scrambled.split("").forEach((char, idx) => {
    if (char === " ") {
      ...
      item.className =
        "px-3.5 py-2 bg-gradient-to-br from-amber-50 to-amber-100 text-amber-800 font-extrabold text-xl rounded-2xl border-2 border-amber-200 uppercase shadow-sm flex items-center justify-center min-w-[40px] transform hover:scale-105 transition-all select-none";
  ...
  // Thiết lập gợi ý
  document.getElementById("scramble-meaning-hint").textContent =
    word.meaning_vi;
  const typeText =
    word.word_type === "n"
      ? "danh từ"
  ...
  document.getElementById("scramble-ipa-hint").textContent =
  ...
  document.getElementById("scramble-feedback").className =
    "hidden p-4 rounded-xl text-sm font-bold text-center";
  ...
  document.getElementById("scramble-timer-box").className =
    "flex items-center gap-1 text-sm font-bold text-amber-600 px-2.5 py-1 bg-amber-50 rounded-full";

  window.studentState.scrambleTimerInterval = setInterval(() => {
    window.studentState.scrambleTimeLeft--;
    document.getElementById("scramble-timer-text").textContent =
      window.studentState.scrambleTimeLeft;

    if (window.studentState.scrambleTimeLeft <= 10) {
      // Đổi sang màu đỏ cảnh báo gấp rút
      document.getElementById("scramble-timer-box").className =
        "flex items-center gap-1 text-sm font-bold text-rose-600 px-2.5 py-1 bg-rose-100 rounded-full animate-bounce";
    }
...
window.submitScrambleAnswer = (e) => {
  ...
  submitBtn.className =
    "w-full py-3 bg-slate-200 text-slate-400 font-bold rounded-xl text-sm cursor-not-allowed";
  ...
  if (isCorrect) {
    window.studentState.gameCorrectAnswers++;
    document.getElementById("scramble-correct-count").textContent =
      window.studentState.gameCorrectAnswers;
...
function loadFitbQuestion(index) {
  ...
  // Cập nhật tiến trình UI
  document.getElementById("fitb-progress-text").textContent =
    `Câu ${index + 1}/${total}`;
  const progressPercent = ((index + 1) / total) * 100;
  document.getElementById("fitb-progress-bar").style.width =
    `${progressPercent}%`;
  ...
  if (boundaryRegex.test(exampleText)) {
    formattedSentence = exampleText.replace(
      boundaryRegex,
      `<span class="px-3 py-1 bg-amber-100 border-2 border-dashed border-amber-400 rounded-lg text-brand-700 font-black tracking-widest font-mono text-sm mx-1">________</span>`,
    );
  } else {
  ...
  document.getElementById("fitb-sentence-container").innerHTML =
    formattedSentence;
      ...
      : word.word_type === "v"
        ...
        : word.word_type === "adj"
          ...
          : word.word_type === "adv"
            ? "trạng từ"
            : "từ vựng";
  document.getElementById("fitb-type-hint").textContent =
  ...
  document.getElementById("fitb-feedback").className =
    "hidden p-4 rounded-xl text-sm font-bold text-center";
...
window.submitFitbAnswer = (e) => {
  ...
  let solvedSentence = exampleText.replace(
    boundaryRegex,
    `<span class="px-3 py-1 bg-emerald-100 border-2 border-emerald-500 rounded-lg text-emerald-800 font-black tracking-normal font-mono text-sm mx-1 animate-pulse">${correctWord}</span>`,
  );
  document.getElementById("fitb-sentence-container").innerHTML = solvedSentence;

  if (isCorrect) {
    window.studentState.gameCorrectAnswers++;
    document.getElementById("fitb-correct-count").textContent =
      window.studentState.gameCorrectAnswers;
...
// ==========================================
window.showGameReport = async (score, total) => {
  ...
  const timeSpent = Math.floor(
    (Date.now() - (window.studentState.gameStartTime || Date.now())) / 1000,
  );

  // Ghi nhận kết quả lên server
  try {
    await window.apiFetch("/api/game-results", {
      method: "POST",
      body: JSON.stringify({
        score: score,
  ...
  if (percent === 100) {
    ...
    badgeEl.className =
      "text-xs font-bold text-slate-700 bg-slate-50 px-3 py-1 rounded-full inline-block mt-3 border border-slate-200";
  }
...
window.loadGameLeaderboards = async () => {
  try {
    ...
    if (playedUl && data.topPlayed) {
      if (data.topPlayed.length > 0) {
        playedUl.innerHTML = data.topPlayed
          .map(
            (u, i) => `
          <li class="px-4 py-3 flex items-center justify-between hover:bg-slate-50 transition-colors">
            <div class="flex items-center gap-3">
              <span class="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 font-bold text-xs flex items-center justify-center">${i + 1}</span>
              <span class="text-sm font-semibold text-slate-800">${u.full_name}</span>
            </div>
            <span class="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100">${u.play_count} lượt</span>
          </li>
        `,
          )
          .join("");
    ...
    if (scoreUl && data.topScore) {
      if (data.topScore.length > 0) {
        scoreUl.innerHTML = data.topScore
          .map(
            (u, i) => `
          <li class="px-4 py-3 flex items-center justify-between hover:bg-slate-50 transition-colors">
            <div class="flex items-center gap-3">
              <span class="w-6 h-6 rounded-full bg-amber-100 text-amber-700 font-bold text-xs flex items-center justify-center">${i + 1}</span>
              <span class="text-sm font-semibold text-slate-800">${u.full_name}</span>
            </div>
            <div class="text-right flex flex-col items-end">
              <span class="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100">${u.score} điểm</span>
              <span class="text-[10px] text-slate-500 font-medium mt-1">${u.time_spent} giây</span>
            </div>
          </li>
        `,
          )
    ...
    console.error("Lỗi tải bảng xếp hạng game:", err);
  }
};
...
window.loadStudentExams = async () => {
  try {
    ...
    if (list.length === 0) {
      grid.innerHTML =
        '<p class="col-span-full text-center text-slate-400 text-sm">Hiện tại chưa có đề thi thử nào được mở.</p>';
      return;
    }
    ...
    list.forEach((e) => {
      ...
      card.className =
        "bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:border-brand-300 hover:shadow-md transition-all space-y-4";
      card.innerHTML = `
    ...
    shuffledGrouped.forEach((g, gIdx) => {
      if (g.type === "single") {
        ...
        g.subQuestions.forEach((subQ) => {
          ...
          subDiv.className =
            "border border-slate-100 rounded-2xl p-5 bg-slate-50/20 space-y-3 transition-all hover:shadow-sm";
          subDiv.innerHTML = `
...
window.selectOption = (questionOrder, answer) => {
  ...
  if (navBubble) {
    navBubble.className =
      "w-10 h-10 bg-brand-600 text-white rounded-lg text-xs font-bold flex items-center justify-center transition-all";
  }
};
...
window.submitExam = async () => {
  ...
  try {
    ...
    document.getElementById("s-exams-taking-screen").classList.add("hidden");
    document
      .getElementById("s-exams-results-screen")
      .classList.remove("hidden");

    document.getElementById("result-screen-exam-title").textContent =
      window.studentState.activeExam.title;
    document.getElementById("result-score").textContent = `${res.score}đ`;
    document.getElementById("result-correct-ratio").textContent =
      `${res.total_correct} / ${res.total_questions}`;

    // Đăng ký nút xem lời giải chi tiết
    document.getElementById("btn-show-solutions").onclick = () => {
      showExamSolutions(
        window.studentState.activeQuestions,
        window.studentState.answers,
      );
    };
...
function showExamSolutions(questions, studentAnswers) {
  ...
  grouped.forEach((g, gIdx) => {
    if (g.type === "single") {
      ...
      const cAns = g.correct_answer
        ? String(g.correct_answer).toUpperCase().trim()
        : "";
      const isCorrect =
        sAns && cAns && String(sAns).toUpperCase().trim() === cAns;
      ...
      const subContainer = groupBlock.querySelector(
        `#solutions-sub-container-${gIdx}`,
      );
      g.subQuestions.forEach((subQ) => {
        const qNum = subQ.original_num;
        const sAns = studentAnswers[qNum] || "";
        const cAns = subQ.correct_answer
          ? String(subQ.correct_answer).toUpperCase().trim()
          : "";
        const isCorrect =
          sAns && cAns && String(sAns).toUpperCase().trim() === cAns;
...
window.loadStudentQna = async () => {
  ...
  try {
    ...
    if (list.length === 0) {
      container.innerHTML =
        '<div class="p-8 text-center text-slate-500 bg-white rounded-xl border border-slate-200">Con chưa có câu hỏi nào. Đừng ngại hỏi Cô Hiền nhé!</div>';
      return;
    }
...
window.analyzeStudentWriting = async () => {
  ...
  try {
    ...
    const htmlOutput = res.analysis
      ...
      .replace(
        /### (.*?)(<br>|<\/p>)/g,
        '<h4 class="text-base font-extrabold text-brand-600 mt-4 mb-2">$1</h4>',
      )
      .replace(
        /- (.*?)(<br>|<\/p>)/g,
        '<li class="ml-4 list-disc text-xs">$1</li>',
      );
